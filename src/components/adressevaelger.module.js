export default function adressevaelgerModule() {
    ...
    return (() => {
        ...
        var E = (r, e) => {
            ...
            if (e && (typeof e == "object" || typeof e == "function")) for (let i of u(e)) !p.call(r, i) && i !== t && o(r, i, {
                get: () => e[i],
        ...
        var y = {};
        E(y, {AdresseSearchAPI: () => n, AdresseSearchInput: () => h, adressevaelger: () => c, default: () => k});
        var n = class {
            static {
            ...
            constructor(e) {
                if (!e || !e.token) throw new Error('AdresseSearchAPI must be initialized with a valid configuration. `{token: "xxx"} is the minimum required.`');
                this.token = e.token;
                e.apiUrl && (this.apiUrl = e.apiUrl)
            }

            async search(e, t, s = {}) {
                if (!e || !t) throw new Error("search() requires both endpoint and query parameters.");
                let i = await fetch(`${this.apiUrl}/${e}/soeg?tekst=${t}&token=${this.token}${this.formatParams(s)}`);
                if (!i.ok) throw new Error(`HTTP error! Status: ${i.status}`);
                let l = await i.json();
                if (l.status === "fejl") throw new Error(`Search error: ${l.beskrivelse}`);
                return l.fund
            }

            async get(e, t) {
                if (!e || !t) throw new Error("get() requires both endpoint and id parameters.");
        ...
        var m = class {
            ...
            constructor(e, t) {
                ...
                this.wrapperElement.addEventListener("keyup", this.listKeyHandler.bind(this));
                document.addEventListener("click", this.outsideClickHandler.bind(this));
                let s = this.options.apiUrl ? {token: t.token, apiUrl: this.options.apiUrl} : {token: t.token};
                this.api = new n(s)
            }

            inputHandler(e) {
                e.target.value !== "" && (this.debounceTimer && clearTimeout(this.debounceTimer));
            ...
            async refreshList(e) {
                try {
                    ...
                    this.errorHandler(new Error(`Failed to load search items: ${t.message}`))
                }
            }
            ...
            errorHandler(e) {
                console.error(e);
                this.inputElement.dispatchEvent(new CustomEvent("address:error", {
                    bubbles: !0,
                    composed: !0,
            ...
            listKeyHandler(e) {
                if (e.key === "ArrowDown") {
                    ...
                    this.moveFocus(-1);
                } else if (e.key === "Enter" && this.listElement.querySelector(":focus")) {
            ...
            outsideClickHandler(e) {
                this.wrapperElement.contains(e.target) || this.listElement.querySelector("ul")?.remove()
            }
            ...
            selectProcessor(e) {
                // there is a doubt on the last parenthesis
                if (e.type === "vejnavn" || e.type === "navngivenvejpostnummer" || (e.type === "husnummer" && this.searchType === "adresser")) {
                    this.inputElement.value = e.titel;
                    this.refreshList(e.titel);
                } else {
                    this.listElement.querySelector("ul")?.remove();
                    this.selectItem(e);
                }
            ...
            async selectItem(e) {
                try {
                    ...
                    this.inputElement.value = e.titel;
                    this.inputElement.dispatchEvent(new CustomEvent("address:select", {
                        bubbles: !0,
                        composed: !0,
                        detail: t
                    }));
        ...
        var h = class extends HTMLElement {
            ...
            static observedAttributes = ["placeholder", "disabled", "adgangsadresser-only", "kommune-kode", "maksimum", "medtag-foreloebige", "token", "api-url"];
  ...
  `;
            ...
            setAPI() {
                ...
                let e = this.options.apiUrl ? {apiUrl: this.options.apiUrl, token: this.token} : {token: this.token};
                this.api = new n(e)
            }
            ...
            async selectProcessor(e) {
                if (e.type === "vejnavn" || e.type === "navngivenvejpostnummer" || (e.type === "husnummer" && this.searchType === "adresser")) {
                    this.inputElement.value = e.titel;
                    await this.refreshList(e.titel);
                } else {
                    await this.selectItem(e);
            ...
            async selectItem(e) {
                try {
                    let t = await this.api.get(this.searchType, e.id);
                    this.inputElement.value = e.titel;
                    this.dispatchEvent(new CustomEvent("address:select", {
                        bubbles: !0,
                        composed: !0,
                        detail: t
                    }));
            ...
            renderInput() {
                ...
                this.inputElement.addEventListener("input", this.inputHandler.bind(this));
                this.inputElement.addEventListener("keyup", this.inputKeyHandler.bind(this));
            ...
            errorHandler(e) {
                console.error(e);
                this.dispatchEvent(new CustomEvent("address:error", {
                    bubbles: !0,
                    composed: !0,
                    detail: {message: e.message}
                }))
